// =============================================================================
// SUMMA_TN_AB_GEMM - 分布式复数矩阵乘法实现
// =============================================================================
// 计算:
//   C = alpha * op(A) * op(B) + beta * C
// 其中:
//   op(A) = A^T,  A 按原始形状 (K x M) 分布和存储
//   op(B) = B,    B 按原始形状 (K x N) 分布和存储
//
// 算法:
//   - 使用 SUMMA broadcast-broadcast 变体
//   - A 的 K-panel 沿进程行广播
//   - B 的 K-panel 沿进程列广播
//   - 本地计算使用 cuBLAS GEMM, transa=T, transb=N
//
// 数据类型:
//   - cuComplex       (complex float)
//   - cuDoubleComplex (complex double)
//
// 推荐构建 (Linux/WSL2):
//   nvcc -O3 -std=c++17 -ccbin mpicxx \
//       -gencode arch=compute_89,code=sm_89 \
//       -o summa_tn_ab summa_tn_ab.cu \
//       -lcublas -lnccl
//
// 示例:
//   mpirun -np 4 ./summa_tn_ab 2 2 4096 4096 4096 1024 1024 1024 cfloat 1 0 0 0 1
// =============================================================================
...
template <typename T>
static T value_a(int64_t k, int64_t i) {
...
template <typename T>
static T value_b(int64_t k, int64_t j) {
    using Traits = ComplexTraits<T>;
    using Real = typename Traits::Real;
...
static void fill_local_A_host(
    ...
    for (int step = 0; step < plan.steps; ++step) {
        ...
        const int64_t local_row0 = plan.a_offsets[step];

        for (int64_t i = 0; i < mloc; ++i) {
            T* dst_col = host_A->data() + static_cast<size_t>(i * plan.a_k_local + local_row0);
            const int64_t gi = row_indices[static_cast<size_t>(i)];
            for (int64_t kk = 0; kk < ks; ++kk) {
                dst_col[kk] = value_a<T>(k0 + kk, gi);
            }
        }
    }
}
...
static void fill_local_B_host(
    ...
    for (int step = 0; step < plan.steps; ++step) {
        ...
        for (int64_t j = 0; j < nloc; ++j) {
            T* dst_col = host_B->data() + static_cast<size_t>(j * plan.b_k_local + local_row0);
            const int64_t gj = col_indices[static_cast<size_t>(j)];
            for (int64_t kk = 0; kk < ks; ++kk) {
                dst_col[kk] = value_b<T>(k0 + kk, gj);
            }
        }
    }
}
...
static void summa_TN_AB_gemm(
    ...
    allocate_matrix(&A_panel, std::max<int64_t>(plan.max_panel, 1), C_local->rows);
    ...
    for (int step = 0; step < plan.steps; ++step) {
        ...
        if (grid.pcol == a_root && A_local.rows > 0 && A_local.cols > 0) {
            ...
            const T* src_A = A_local.d_ptr + a_offset;
            CUDA_CHECK(cudaMemcpy2DAsync(
                A_panel.d_ptr,
                static_cast<size_t>(A_panel.ld) * sizeof(T),
                src_A,
                static_cast<size_t>(A_local.ld) * sizeof(T),
                static_cast<size_t>(panel_k) * sizeof(T),
                static_cast<size_t>(A_local.cols),
        ...
        if (grid.prow == b_root && B_local.rows > 0 && B_local.cols > 0) {
            ...
            const T* src_B = B_local.d_ptr + b_offset;
            CUDA_CHECK(cudaMemcpy2DAsync(
                B_panel.d_ptr,
        ...
        const size_t a_count = static_cast<size_t>(C_local->rows * panel_k);
        const size_t b_count = static_cast<size_t>(B_local.cols * panel_k);
    ...
    for (int64_t j = 0; j < N; ++j) {
        for (int64_t i = 0; i < M; ++i) {
            ...
            for (int64_t k = 0; k < K; ++k) {
                acc += Traits::to_std(value_a<T>(k, i)) * Traits::to_std(value_b<T>(k, j));
            }
...
static void print_usage(int rank) {
    ...
    printf("Usage:\n");
    printf("  mpirun -np <nprow*npcol> ./summa_tn_ab "
    ...
    printf("\n");
    printf("Examples:\n");
    printf("  mpirun -np 4 ./summa_tn_ab 2 2 4096 4096 4096 1024 1024 1024 cfloat 1 0 0 0 0\n");
    printf("  mpirun -np 4 ./summa_tn_ab 2 2 512 512 512 128 128 128 cdouble 1 0 0 0 1\n");
}
...
static int run_summa(
    ...
    LocalMatrix<T> C_local;
    allocate_matrix(&A_local, plan.a_k_local, static_cast<int64_t>(row_indices.size()));
    ...
    if (grid.world_rank == 0) {
        printf("========================================\n");
        printf("SUMMA_TN_AB_GEMM (MPI + NCCL + cuBLAS)\n");
        printf("========================================\n");
        printf("Process grid : %d x %d = %d ranks\n", nprow, npcol, grid.world_size);
        printf("Data type    : %s\n", Traits::name());
        printf("Matrix size  : op(A^T from %lld x %lld) * B(%lld x %lld) -> C(%lld x %lld)\n",
        ...
        for (int rank = 0; rank < grid.world_size; ++rank) {
            ...
            printf("rank %d -> grid(%d,%d) gpu=%lld  Araw(%lld x %lld)  B(%lld x %lld)  C(%lld x %lld)\n",
                   rank,
                   prow,
                   pcol,
                   shape[6],
